-- NeatCraftingControllerSupport: NC_RecipeList_Panel Patch
-- 手柄导航配方列表
...
-- 只在 OnGameBoot 时注入方法（确保 Neat_Crafting 已加载）
local function injectRecipeListPanelMethods()
    -- 如果 NC_RecipeList_Panel 不存在或已经有方法，跳过
    if not NC_RecipeList_Panel or NC_RecipeList_Panel._joypadMethodsInjected then return end
    NC_RecipeList_Panel._joypadMethodsInjected = true

    -- 保存原始方法
    NC_RecipeList_Panel._originalOnJoypadDown = NC_RecipeList_Panel.onJoypadDown
    NC_RecipeList_Panel._originalOnJoypadDirDown = NC_RecipeList_Panel.onJoypadDirDown
...
local NC_RecipeList_Panel_patch = {
    inject = injectRecipeListPanelMethods
}
